...
int remote_query(const CLP &cmd)
{
    ...
    for(auto i:items_vec){
        ...
        // std::cout << "items_vec = " << oc::toBlock(i.get_as<uint64_t>()[0],i.get_as<uint64_t>()[1]) << std::endl;
    }
        
    // for(auto i:orig_items){
    //     std::cout << "orig_items = " << oc::toBlock((uint8_t*)i.data()) << std::endl;
    // }

    vector<MatchRecord> query_result;
    try {
        APSU_LOG_INFO("Sending APSU query");
        query_result = sender.request_query(items_without_OPRF,  channel, orig_items);
    ...
    //     sender.ResponseOT(cmd.net_addr());
